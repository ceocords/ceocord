...
const commonOptions = {
    ...commonOpts,
    entryPoints: ["browser/CeoCord.ts"],
    format: "iife",
    globalName: "CeoCord",
...
const buildConfigs = [
    ...
    },
    {
        ...
        footer: { js: "//# sourceURL=file:///CeoCordWeb" }
    },
    {
        ...commonOptions,
    ...
    },
    {
        ...
        outfile: "dist/CeoCord.user.js",
        banner: {
            js: readFileSync("browser/userscript.meta.js", "utf-8").replace("%version%", `${VERSION}.${new Date().getTime()}`)
        },
        footer: {
            // UserScripts get wrapped in an iife, so define CeoCord prop on window that returns our local
            js: "Object.defineProperty(unsafeWindow,'CeoCord',{get:()=>CeoCord});"
        }
    }
];
...
async function buildExtension(target, files) {
    const entries = {
        "dist/CeoCord.js": await readFile("dist/extension.js"),
        "dist/CeoCord.css": await readFile("dist/extension.css"),
...
const appendCssRuntime = readFile("dist/CeoCord.user.css", "utf-8").then(content => {
    const cssRuntime = `unsafeWindow._vcUserScriptRendererCss=\`${content.replaceAll("`", "\\`")}\``;

    return appendFile("dist/CeoCord.user.js", cssRuntime);
});
