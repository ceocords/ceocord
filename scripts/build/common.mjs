...
export const gitHash = process.env.CEOCORD_HASH || execSync("git rev-parse --short HEAD", { encoding: "utf-8" }).trim();
...
export const gitRemotePlugin = {
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "git-remote" }, async () => {
            let remote = process.env.CeoCord_REMOTE;
            if (!remote) {
                try {
...
export const commonOpts = {
    ...
    jsx: "transform",
    jsxFactory: "CeoCordCreateElement",
    jsxFragment: "CeoCordFragment"
};
