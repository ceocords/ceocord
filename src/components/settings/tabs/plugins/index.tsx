...
function ExcludedPluginsList({ search }: { search: string; }) {
    ...
    const ExcludedReasons: Record<"web" | "discordDesktop" | "vesktop" | "desktop" | "dev", string> = {
        ...
        dev: "Developer version of CeoCord"
    };
...
function PluginSettings() {
    ...
    const [newPlugins] = useAwaiter(() => DataStore.get("CeoCord_existingPlugins").then((cachedPlugins: Record<string, number> | undefined) => {
        ...
        DataStore.set("CeoCord_existingPlugins", existingTimestamps);
    ...
    for (const p of sortedPlugins) {
        ...
        if (isRequired) {
            const tooltipText = p.required || !depMap[p.name]
                ? "This plugin requires CeoCord to function."
