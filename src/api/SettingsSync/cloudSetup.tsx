...
export async function checkCloudUrlCsp() {
    ...
    if (host === "api.CeoCord.dev") return true;

    if (await CeoCordNative.csp.isDomainAllowed(Settings.cloud.url, ["connect-src"])) {
        return true;
    }

    const res = await CeoCordNative.csp.requestAddOverride(Settings.cloud.url, ["connect-src"], "Cloud Sync");
    if (res === "ok") {
        Alerts.show({
...
export async function getAuthorization() {
    const secrets = await DataStore.get<Record<string, string>>("CeoCord_cloudSecret") ?? {};

    const origin = getCloudUrlOrigin();

    // we need to migrate from the old format here
    if (secrets[origin]) {
        await DataStore.update<Record<string, string>>("CeoCord_cloudSecret", secrets => {
            secrets ??= {};
