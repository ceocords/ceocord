...
export async function importSettings(data: string) {
    ...
    if ("settings" in parsed && "quickCss" in parsed) {
        ...
        await CeoCordNative.settings.set(parsed.settings);
        await CeoCordNative.quickCss.set(parsed.quickCss);
    } else
        throw new Error("Invalid Settings. Is this even a CeoCord Settings file?");
}

export async function exportSettings({ minify }: { minify?: boolean; } = {}) {
    const settings = CeoCordNative.settings.get();
    const quickCss = await CeoCordNative.quickCss.get();
...
export async function uploadSettingsBackup(showToast = true): Promise<void> {
    if (IS_DISCORD_DESKTOP) {
        const [file] = await DiscordNative.fileManager.openFiles({
            filters: [
                { name: "CeoCord Settings Backup", extensions: ["json"] },
                { name: "all", extensions: ["*"] }
            ]
        });
