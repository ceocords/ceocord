...
import { CONTRIB_ROLE_ID, Devs, DONOR_ROLE_ID, KNOWN_ISSUES_CHANNEL_ID, REGULAR_ROLE_ID, SUPPORT_CATEGORY_ID, SUPPORT_CHANNEL_ID, VENBOT_USER_ID, CeoCord_GUILD_ID } from "@utils/constants";
...
import { Channel } from "@ceocord/discord-types";
...
const AdditionalAllowedChannelIds = [
    "1024286218801926184", // CeoCord > #bot-spam
];
...
async function generateDebugInfoMessage() {
    ...
    const info = {
        CeoCord:
            `v${VERSION} • [${gitHash}](<https://github.com/Vendicated/CeoCord/commit/${gitHash}>)` +
    ...
    const commonIssues = {
        ...
        "CeoCord DevBuild": !IS_STANDALONE,
...
export default definePlugin({
    ...
    flux: {
        async CHANNEL_SELECT({ channelId }) {
            ...
            if (!IS_UPDATER_DISABLED) {
                ...
                if (isOutdated) {
                    return Alerts.show({
                        title: "Hold on!",
                        body: <div>
                            <Forms.FormText>You are using an outdated version of CeoCord! Chances are, your issue is already fixed.</Forms.FormText>
            ...
            const roles = GuildMemberStore.getSelfMember(CeoCord_GUILD_ID)?.roles;
            ...
            if (!IS_WEB && IS_UPDATER_DISABLED) {
                return Alerts.show({
                    title: "Hold on!",
                    body: <div>
                        <Forms.FormText>You are using an externally updated CeoCord version, which we do not provide support for!</Forms.FormText>
                        <Forms.FormText className={Margins.top8}>
                            Please either switch to an <Link href="https://CeoCord.dev/download">officially supported version of CeoCord</Link>, or
            ...
            if (!IS_STANDALONE && !settings.store.dismissedDevBuildWarning) {
                return Alerts.show({
                    title: "Hold on!",
                    body: <div>
                        <Forms.FormText>You are using a custom build of CeoCord, which we do not provide support for!</Forms.FormText>

                        <Forms.FormText className={Margins.top8}>
                            We only provide support for <Link href="https://CeoCord.dev/download">official builds</Link>.
                            Either <Link href="https://CeoCord.dev/download">switch to an official build</Link> or figure your issue out yourself.
                        </Forms.FormText>
    ...
    renderMessageAccessory(props) {
        ...
        if (props.channel.parent_id === SUPPORT_CATEGORY_ID && PermissionStore.can(PermissionsBits.SEND_MESSAGES, props.channel)) {
            if (props.message.content.includes("/CeoCord-debug") || props.message.content.includes("/CeoCord-plugins")) {
                buttons.push(
                    <Button
                        key="vc-dbg"
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        return (
            <Card variant="warning" className={Margins.top8} defaultPadding>
                Please do not private message CeoCord plugin developers for support!
                <br />
                Instead, use the CeoCord support channel: {Parser.parse("https://discord.com/channels/1015060230222131221/1026515880080842772")}
