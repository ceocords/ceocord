...
import { BackupAndRestoreTab, CeoCordTab, CloudTab, PatchHelperTab, PluginsTab, ThemesTab, UpdaterTab } from "@components/settings/tabs";
...
const settings = definePluginSettings({
    settingsLocation: {
        ...
        description: "Where to put the CeoCord settings section",
        options: [
...
export default definePlugin({
    ...
    buildLayout(originalLayoutBuilder: SettingsLayoutBuilder) {
        ...
        if (layout.some(s => s?.key === "CeoCord_section")) return layout;

        const { buildEntry } = this;

        const CeoCordEntries: SettingsLayoutNode[] = [
            buildEntry({
                key: "CeoCord_main",
                title: "CeoCord",
                panelTitle: "CeoCord Settings",
                Component: CeoCordTab,
            ...
            buildEntry({
                key: "CeoCord_cloud",
                title: "Cloud",
                panelTitle: "CeoCord Cloud",
                Component: CloudTab,
                Icon: CloudIcon
            }),
            buildEntry({
                key: "CeoCord_backup_restore",
                title: "Backup & Restore",
                Component: BackupAndRestoreTab,
                Icon: BackupRestoreIcon
            }),
            IS_DEV && PatchHelperTab && buildEntry({
                key: "CeoCord_patch_helper",
                title: "Patch Helper",
            ...
            ...this.customSections.map((func, i) => {
                ...
                return buildEntry({
                    key: `CeoCord_deprecated_custom_${section}`,
                    title: label,
                    Component: element,
        ...
        const CeoCordSection: SettingsLayoutNode = {
            key: "CeoCord_section",
            type: LayoutTypes.SECTION,
            useTitle: () => "CeoCord Settings",
            buildLayout: () => CeoCordEntries
        };
        ...
        layout.splice(idx, 0, CeoCordSection);
    ...
    get electronVersion() {
        return CeoCordNative.native.getVersions().electron || window.legcord?.electron || null;
    },

    get chromiumVersion() {
        try {
            return CeoCordNative.native.getVersions().chrome
    ...
    getInfoRows() {
        ...
        const rows = [`CeoCord ${gitHash}${additionalInfo}`];
