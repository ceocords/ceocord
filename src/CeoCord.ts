...
async function syncSettings() {
    if (localStorage.CeoCord_cloudSyncDirection === undefined) {
        // by default, sync bi-directionally
        localStorage.CeoCord_cloudSyncDirection = "both";
    }

    // pre-check for local shared settings
    if (
        Settings.cloud.authenticated &&
        !await dsGet("CeoCord_cloudSecret") // this has been enabled due to local settings share or some other bug
    ) {
        ...
        showNotification({
            ...
            onClick: () => SettingsRouter.open("CeoCordCloud")
        });
        return;
    }
    ...
    if (
        ...
        localStorage.CeoCord_cloudSyncDirection !== "manual" // if we're not in manual mode
    ) {
        if (localStorage.CeoCord_settingsDirty && shouldCloudSync("push")) {
    ...
    SettingsStore.addGlobalChangeListener(() => {
        localStorage.CeoCord_settingsDirty = true;
    ...
    try {
        ...
        if (Settings.autoUpdate) {
            ...
            if (Settings.autoUpdateNotification) {
                notify({
                    title: "CeoCord has been updated!",
        ...
        notify({
            title: "A CeoCord update is available!",
    ...
    if (IS_DEV) {
        ...
        if (pendingPatches.length)
            PMLogger.warn(
                ...
                "You are seeing this warning because this is a Development build of CeoCord.",
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (IS_DISCORD_DESKTOP && Settings.winNativeTitleBar && IS_WINDOWS) {
        createAndAppendStyle("ceocord-native-titlebar-style", coreStyleRootNode).textContent = "[class*=titleBar]{display: none!important}";
    }
}, { once: true });
