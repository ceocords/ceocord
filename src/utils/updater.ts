...
export async function checkForUpdates() {
    changes = await Unwrap(CeoCordNative.updater.getUpdates());
...
export async function update() {
    ...
    const res = await Unwrap(CeoCordNative.updater.update());

    if (res) {
        isOutdated = false;
        if (!await Unwrap(CeoCordNative.updater.rebuild()))
...
export const getRepo = () => Unwrap(CeoCordNative.updater.getRepo());
