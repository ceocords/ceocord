...
const setCssDebounced = debounce((css: string) => CeoCordNative.quickCss.set(css));

const themeStore = DataStore.createStore("CeoCordThemes", "CeoCordThemeData");

// probably should make this less cursed at some point
window.CeoCordNative = {
    themes: {
    ...
    updater: {
        getRepo: async () => ({ ok: true, value: "https://github.com/Vendicated/CeoCord" }),
    ...
    quickCss: {
        get: () => DataStore.get("CeoCordQuickCss").then(s => s ?? ""),
        set: async (css: string) => {
            await DataStore.set("CeoCordQuickCss", css);
        ...
        async openEditor() {
            ...
            const win = open("about:blank", "CeoCordQuickCss", features);
            if (!win) {
            ...
            win.getCurrentCss = () => CeoCordNative.quickCss.get();
    ...
    settings: {
        get: () => {
            try {
                return JSON.parse(localStorage.getItem("CeoCordSettings") || "{}");
            } catch (e) {
                console.error("Failed to parse settings from localStorage: ", e);
                return {};
            }
        },
        set: async (s: Settings) => localStorage.setItem("CeoCordSettings", JSON.stringify(s)),
